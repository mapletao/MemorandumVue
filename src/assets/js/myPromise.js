...
function initParentPromise (myPromise, resolve) {
  return function (resolve, myPromise, val) {
    ...
    initMyPromiseComplete(myPromise, resolve === _resolve)
...
function then (resolve, reject) {
  const myPromise = new this.constructor(noop)
  if (!resolve && !reject) {
    return this
  }
...
function _race (vals) {
  ...
  const completeHandle = (msg, cb) => {
    ...
    nextTick(initMyPromiseComplete.bind(null, myPromiseRace), resolve === _resolve)
  }
  if (!_isArray(vals)) {
